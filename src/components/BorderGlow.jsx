...
const BorderGlow = ({
  ...
  }, [getEdgeProximity, getCursorAngle, rotateOnHover]);
  ...
  useEffect(() => {
    ...
    const rotate = () => {
      setCursorAngle((angle) => (angle + rotationSpeed) % 360);
  ...
  }, [isHovered, rotateOnHover, rotationSpeed]);

  const colorSensitivity = edgeSensitivity + 20;
  const isVisible = isHovered || sweepActive;
  const activeBorderOpacity = isVisible
    ? Math.max(0, (edgeProximity * 100 - colorSensitivity) / (100 - colorSensitivity))
    : 0;
  const borderOpacity = Math.max(idleBorderOpacity, activeBorderOpacity);
  ...
  return (
    <div
      ...
      onPointerLeave={() => {
        setIsHovered(false);
        if (rotateOnHover) setEdgeProximity(0);
      }}
      className={`relative grid isolate ${className}`}
      style={{
        background: backgroundColor,
        border: `${borderWidth}px solid rgba(255, 255, 255, 0.14)`,
